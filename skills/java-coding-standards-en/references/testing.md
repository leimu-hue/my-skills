...
- [ ] Test names express scenario and expectation
- [ ] Uses Given / When / Then or equivalent clear structure
- [ ] Success, failure, and boundary paths covered
- [ ] Mock boundaries are reasonable; no over-verification of internals
- [ ] Assertions verify behavior, not just non-null

### Integration Tests

- [ ] Chose the minimal viable Spring test scope
- [ ] Test data isolated and cleanable
- [ ] Verifies HTTP status, response body, and key results in DB or external interactions
- [ ] Not dependent on test order or shared state

### Stability

- [ ] Not dependent on real time, random numbers, external networks, or execution order
- [ ] Async tests don't use fixed `sleep`
- [ ] Default test suite execution speed is acceptable
- [ ] Coverage targets follow project CI, not blind number-chasing
