...
- [ ] External input validated for whitelist, length, and range
- [ ] SQL values use parameter binding
- [ ] MyBatis `${}` only uses server-side whitelist mapping results
- [ ] Output encoded per HTML, attribute, JS, and URL context
- [ ] File upload and path access have boundary checks

### Rate Limiting and Abuse Prevention

- [ ] High-risk entry points have rate limiting
- [ ] Rate limit thresholds differ by scenario
- [ ] Rate limit failures return 429; logs include source identifier

### Identity and Permissions

- [ ] Default deny, explicit authorization
- [ ] Both interface and object-level data permissions are checked
- [ ] CSRF strategy matches authentication method
- [ ] Cookies, sessions, and tokens have security attributes and lifetimes

### Sensitive Data

- [ ] Passwords use BCrypt, Argon2, or PBKDF2
- [ ] Keys are not hardcoded
- [ ] Reversible encryption uses unified security component or authenticated encryption mode
- [ ] No plaintext sensitive information in logs, exceptions, or responses

### Audit

- [ ] Key security events are logged
- [ ] Audit log fields are queryable
- [ ] Logs have retention periods and access controls
