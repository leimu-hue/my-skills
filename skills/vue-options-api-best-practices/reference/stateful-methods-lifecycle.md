...
If you use strict TypeScript, instance properties created in `created()` may need an explicit declaration so `this.debouncedSearch` is recognized by the component type system. Common options are declaring the member via component instance augmentation or keeping the debounced function inside an already-typed data or service object.
